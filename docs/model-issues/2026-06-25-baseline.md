...
> **Addendum 2026-09-04 — what changed since this snapshot.** The findings below are kept verbatim
> as the 2026-06-25 baseline; only the points listed here are superseded.
>
> - **Issue 1 — aftercare ids superseded.** The aftercare model was rewritten on `dev` after the
>   AMED workshop (merge 952307f, 2026-07-08); `Gateway_04r56n3` and `Gateway_1tvjymy` no longer
>   exist. The refined model has **four** OR-gateways, all unnamed:
>   ‹unnamed InclusiveGateway› (`Gateway_0jb99j6`), ‹unnamed InclusiveGateway› (`Gateway_1ea5n5l`),
>   ‹unnamed InclusiveGateway› (`Gateway_0zwqhx7`), ‹unnamed InclusiveGateway› (`Gateway_0fz5j0x`).
>   The treatment ids (`Gateway_18pkxio`, `Gateway_19kiq8e`) are unchanged. **Repository total:
>   13 OR-gateways** — treatment 2, aftercare 4, palliative-care 7 (the latter in
>   [`2026-06-29-palliative-care.md`](2026-06-29-palliative-care.md), Issue P1).
> - **Issue 6 — aftercare ids superseded likewise.** `Event_1f3bojn` and `Event_08adr55` are gone;
>   the rewritten model's `intermediateCatchEvent`s are "Warten bis Nachsorgetermin"
>   (`Event_1lwreze`) and "Wartezeit Nachsorge-intervall" (`Event_0q69do5`) — still the element
>   type the soundness analyzer does not support (ADR-0003). The `overarching` figures are
>   unchanged: **5 start events** (SYN-2) — 4 at the top level (`Process_1`: `Event_0ae2j6h`,
>   `Event_0tdpt7a`, `Event_0lk7fnw`, `Event_05n66xb`) + 1 inside the "Staging [LC SoS 2.2]"
>   sub-process (SYN-2 is checked per level).
> - **Model set:** 10 (overarching + 9 sub-pathways) — 7 at this snapshot, 8 from `v0.2.0-rc.1`
>   (screening, same day), 9 since `v0.3.0-rc.1` (palliative-care), 10 since 2026-09-04
>   (initial-entry, #77; not yet in a tagged release). Issue 4's prefix split now reads
>   (`npm run check:metrics`): `bpmn2:` on `aftercare`, `initial-entry`, `overarching`,
>   `palliative-care`; `bpmn:` on `diagnostic`, `molecular-tumor-board`, `patient-consultation`,
>   `treatment`, `tumor-board`; `screening` uses the default namespace (no prefix at all).
> - **Newer findings:** [`2026-06-29-palliative-care.md`](2026-06-29-palliative-care.md),
>   [`2026-09-04-xsd-core-extension-placement.md`](2026-09-04-xsd-core-extension-placement.md).
