...
description: Advisory, read-only review of a lung-cancer BPMN pathway against the acceptance-test clinical (SEM) and pragmatic (PRA) criteria. Use to prepare evidence for the clinical/pragmatic acceptance-test gates. Produces findings only — it NEVER returns a pass/fail and never substitutes for the human consensus (SEM-6) or joint walkthrough (PRA-1).
---
...
> (**SEM-6**). Surface findings as candidates and **report** any BPMN-XML
...
findings for these acceptance-test criteria:

| Acceptance-test criterion | What to look for | Output |
|---|---|---|
...
| **PRA-3** relevance | elements that look superfluous for the acceptance-test purpose | candidates (gentle) |
| **SYN-3** (final, labels) | verb-object label deviations bpmnlint cannot judge (German clinical grammar) | candidates |

## Hard rules

- **Never** output an acceptance-test decision, gate verdict, or a ✓/✗ on any criterion.
